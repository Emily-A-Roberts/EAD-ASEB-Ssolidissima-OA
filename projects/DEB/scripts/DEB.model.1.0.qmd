...
abstract: |
 I'm running Emilien's code developed for Pousse et al. 2023 model simulations. 
...
```{r}
dir_data <- "~/GitHub/EAD-ASEB-Ssolidissima-OA/data/DEB_model_data/"
library(R.matlab)
...
Param$MpHl <- 700 #uatm
Param$MpHh <- 1300 #uatm
```

I'm taking these values directly from the paper because they aren't part of the code - ER 12/2/23

```{r}
Param$Lp <- 3 #cm From Pousse et al. 2023
Param$dv <- 0.09 
Param$RhoE <- 19600 #J/g
Param$RhoV <-  10000 #This is a dummy number I need this number. It is missing. 
...
```{r}
Result<-array(NA, dim=c(ndt, 20, nbAnim)) # Make a 3D matrix
...
n <- 1
    ...
    if(Param$Kappa*Pc1<=PM){# Maintenance costs cannot be paid
      ...
      Pl2=1/Param$GammaL2*(PM-(Param$Kappa*Pc1)+Pr)
      Pr=0
      if(V>=Vp&Er>=Pl2){
        Er=max(0, Er  - Pr-Pl2 * pdt)
        V  = max(0, V)
      }else{
        Pl1=1/Param$GammaL1*(PM-Param$Kappa*Pc1-((1-Param$Kappa)*Pc1-Pj))
        Er=0
