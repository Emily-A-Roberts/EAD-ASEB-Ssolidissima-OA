...
```{r}
    ...
    cT_out1 <- as.vector(Result.check[,8,1][1:ndt])
    cT_out2 <- as.vector(Result.check[,8,2][1:ndt])
    cT_out3 <- as.vector(Result.check[,8,3][1:ndt])

plot(cT_out1~cT_out3, pch = ".")
...
MLE.E <- optim(
      ...
      upper=c(100,100),
      method="L-BFGS-B",
      data_len = Growth.E$Lf[Growth.E$Collection.month==Month.collected]/10,
      fn = NLL_Xk, 
      Food=Food, 
      Param=Param, 
      ndt=ndt, 
...
MLE.P <- optim(
      ...
      method="L-BFGS-B",
      fn = NLL_Xk, 
      data_len=Growth.P$Lf[Growth.P$Collection.month==Month.collected]/10, 
      Init = Init, Food=Food, 
      Temp=Temp, 
      ndt=ndt, 
      Param=Param, 
...
Growth.obs.P <- (Growth.P$Lf[Growth.P$Collection.month==Month.collected]-
                   Growth.P$Li[Growth.P$Collection.month==Month.collected])/10

Growth.obs.E <- (Growth.E$Lf[Growth.E$Collection.month==Month.collected]-
                   Growth.E$Li[Growth.E$Collection.month==Month.collected])/10

plot(Growth.pred.P~Growth.obs.P, ylim = c(0,3), xlim = c(0,3), col = "orange", pch = 21, ylab = "Predicted growth (cm)", xlab = "Observed growth (cm)")
points(Growth.pred.E~Growth.obs.E,  col = "green", pch = 21)
