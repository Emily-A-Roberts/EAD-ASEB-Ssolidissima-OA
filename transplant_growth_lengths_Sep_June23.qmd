---
title: "transplant_growth_lengths_Sep_Dec22"
format: html
editor: visual
---
...
setwd("~/Documents/GitHub/EAD-ASEB-Ssolidissima-OA/data/Growth")
growth <- read.csv(file = "Prelim_June_transplant_data.csv", stringsAsFactors = F)
#growth <- growth[growth$Buried_Dec=="N",]
str(growth)
growth <- growth[!is.na(growth$Site),]
growth$Site[growth$Site=="Ptown"] <- rep("Provincetown",length.out = nrow(growth))
growth$Site[growth$Site=="Eel Pond"] <- rep("Eel Pond",length.out = nrow(growth))

growth$Site <- as.factor(growth$Site)
head(growth)
growth1 <- growth[!is.na(growth$Live),]
growth2 <- growth1[growth1$Live!="",]
plot(growth2$Growth.Increment~growth2$Site, ylim = c(0,40), xlab = "Site", ylab = "Total change in length Sep to June (mm)")
...
summary(aov(growth2$Growth.Increment~growth2$Site))
```
